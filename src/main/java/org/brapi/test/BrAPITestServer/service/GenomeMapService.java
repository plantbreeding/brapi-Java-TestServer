...
public class GenomeMapService {
	...
	public List<GenomeMap> getMapSummaries(String speciesId, String commonCropName, String scientificName, String type,
			Metadata metaData) {
	...
	public GenomeMapDetails getMapDetail(Metadata metaData, String mapDbId) {
		...
		if (entityOption.isPresent()) {
			...
			List<LinkageGroup> groups = page.stream().map((linkageGroupEntity) -> {
				...
				if (linkageGroupEntity.getMarkers() != null)
	...
	public List<MarkerSummaryMap> getMapPositions(String mapDbId, String linkageGroupName, Metadata metaData) {
		return getMapPositionEntities(mapDbId, linkageGroupName, null, null, metaData).stream()
				.map(this::convertFromEntityToSummaryMap).collect(Collectors.toList());
	}

	public List<MarkerSummaryLinkageGroup> getMapPositions(String mapDbId, String linkageGroupName, Integer min,
			Integer max, Metadata metaData) {
		...
		PagingUtility.calculateMetaData(metaData, page);
		List<MarkerSummaryLinkageGroup> data = page.stream().filter((entity) -> {
			return positionFilter(entity.getLocation().toString(), minPosition, maxPosition);
		}).map(this::convertFromEntityToSummaryLinkageGroup).collect(Collectors.toList());
		return data;
	}

	private Page<MarkerEntity> getMapPositionEntities(String mapDbId, String linkageGroupName, Integer min, Integer max,
			Metadata metaData) {
		...
		if (nameb && minb && maxb) {
			page = markerRepository
					.findAllByLinkageGroup_GenomeMapDbIdAndLinkageGroup_LinkageGroupNameAndLocationGreaterThanEqualAndLocationLessThanEqual(
							mapDbId, linkageGroupName, min, max, pageReq);
		} else if (nameb && minb && !maxb) {
			page = markerRepository
					.findAllByLinkageGroup_GenomeMapDbIdAndLinkageGroup_LinkageGroupNameAndLocationGreaterThanEqual(
							mapDbId, linkageGroupName, min, pageReq);
		} else if (nameb && !minb && maxb) {
			page = markerRepository
					.findAllByLinkageGroup_GenomeMapDbIdAndLinkageGroup_LinkageGroupNameAndLocationLessThanEqual(
							mapDbId, linkageGroupName, max, pageReq);
		} else if (nameb && !minb && !maxb) {
			page = markerRepository.findAllByLinkageGroup_GenomeMapDbIdAndLinkageGroup_LinkageGroupName(mapDbId,
					linkageGroupName, pageReq);
		} else if (!nameb && minb && maxb) {
			page = markerRepository
					.findAllByLinkageGroup_GenomeMapDbIdAndLocationGreaterThanEqualAndLocationLessThanEqual(mapDbId,
							min, max, pageReq);
		} else if (!nameb && minb && !maxb) {
			page = markerRepository.findAllByLinkageGroup_GenomeMapDbIdAndLocationGreaterThanEqual(mapDbId, min,
					pageReq);
		} else if (!nameb && !minb && maxb) {
